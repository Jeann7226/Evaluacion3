...
public class MangaServices {
    ...
    public MangaDTO convertirMangaDTO(Manga manga) {
       ...
       // CORRECCIÓN TOTAL:
    try {
        // Ya no necesitas buscar por ID, ¡JPA ya tiene el objeto cargado!
        if (manga.getDemografia() != null) {
            dto.setInformacionExtra(manga.getDemografia().getNombreDemografia());
        } else {
            dto.setInformacionExtra("Sin demografía asignada");
        }
    } catch (Exception e) {
        // CORRECCIÓN DE SINTAXIS: Sin los dos puntos
