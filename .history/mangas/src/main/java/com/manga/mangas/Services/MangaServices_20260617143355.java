...
public class MangaServices {
    ...
    public MangaDTO convertirMangaDTO(Manga manga){
        ...
        try {
            String informacionExtra = webClientBuilder.build()
                .get()
                .uri("http://localhost:8082/api/v1/autores/buscar-por-manga/" + manga.getIdManga())
                .retrieve()
                .bodyToMono(String.class)
                .block();
            dto.setInformacionExtra(informacionExtra);
        } catch (Exception e) {
            dto.setInformacionExtra("Desconectado (manga-services offline o no existe)");
        }
